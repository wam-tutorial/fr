...
if (file.exists("data/wamtuto-files.zip")) {
  file.remove("data/wamtuto-files.zip")
}
zip("data/wamtuto-files.zip", c(datf$link, code$link, r_obj$link))
```

## Zip tout en un

Tous les fichiers de données et de code dans un [**fichier zip**](./data/wamtuto-files.zip)
...
#| output: asis
#| cache: false
apply(r_obj, 1, function(x) paste0("[", x[1], "](", x[2], ")")) |>
  ui_list()
```
